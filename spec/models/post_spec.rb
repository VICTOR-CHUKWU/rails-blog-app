...
RSpec.describe Post, type: :model do
  subject do
    ...
    post = Post.new(title: 'This is a post',
                    ...
                    user: user, comment_count: 0, likes_count: 0)
    post
  end

  describe 'validation tests' do
    it 'fails to create a post without a user' do
      post = Post.new(title: 'afsdfds', text: 'this is a new post', comment_count: 0, likes_count: 0)
  ...
  describe 'method tests' do
    it 'creates a post correctly with the user' do
      post = Post.new(title: 'afsdfds', text: 'this is a new post', comment_count: 0, likes_count: 0)
      post.build_user(name: 'Michael', photo: '', bio: '', post_count: 0)
    ...
    it 'updates the post_counter correctly' do
      ...
      Post.create!(title: 'Post 1ere', text: 'this is my post', comment_count: 0, likes_count: 0, user: user)
      Post.create!(title: 'Second Post', text: 'this is my post', comment_count: 0, likes_count: 0, user: user)
      Post.create!(title: 'Third Post', text: 'this is my post', comment_count: 0, likes_count: 0, user: user)
      Post.create!(title: 'Fourth Post', text: 'this is my post', comment_count: 0, likes_count: 0, user: user)
      user.post_count = user.posts.length
      expect(user.post_count).to eql(4)
    end

    it 'gets the 5 most recent comments for a post' do
      post = Post.new(title: 'My Post', text: 'this is a post', comment_count: 0, likes_count: 0)
      ...
      recent_five = post.find_five_recent_comments
